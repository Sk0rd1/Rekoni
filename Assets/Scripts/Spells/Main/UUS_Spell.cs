...
public class UUS_Spell : SpellUniversal
{
    ...
    private string effectName = "UUS/Debuff";
    private string particlName = "UUS/Explosion";
            ...
            if(eh.IsDeath())
            {
                ...
                foreach (GameObject go in gos) 
                {
                    if (Vector3.Distance(go.transform.position, enemy.transform.position) <= damageRadius * damageRadius)
                    {
                        ...
                        ehc.Damage(1000, TypeDamage.Necrotic);
                        //тут повинно бути замість 1000 якийсь % від maks hp 
                    }
                }
                break;
            }
